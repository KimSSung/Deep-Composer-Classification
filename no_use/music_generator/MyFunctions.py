...
from tensorflow.compat.v1.nn.rnn_cell import DropoutWrapper, BasicLSTMCell, LSTMStateTuple, MultiRNNCell
...
def LSTM_TimeWise_Training_Layer(input_data, state_init, output_keep_prob=1.0):
  ...
  """  
    ...
    multi_lstm_cell = MultiRNNCell(cell_list, state_is_tuple=True)
    ...
    # softmax added
    X_for_softmax = tf.reshape(outputs, [-1, num_states[-1]])
    softmax_w = tf.compat.v1.get_variable("softmax_w", [num_states[-1], num_classes])
    softmax_b = tf.compat.v1.get_variable("softmax_b",[num_classes])
    output = tf.matmul(X_for_softmax,softmax_w) + softmax_b

    output = tf.reshape(output, shape=[batch_size, num_notes, num_timesteps, num_states[-1]])
...
def LSTM_NoteWise_Layer(input_data, state_init, output_keep_prob=1.0):
    ...
    for n in range(num_notes):    
        ...
        y_n = dense(inputs=h_final_out, units=2, activation=None)
        #print('y_n shape = ', y_n.get_shape())
        
        # sample note play/articulation using Probability of event = sigmoid(y_n)
        note_gen_n = tf.compat.v1.distributions.Bernoulli(logits=y_n).sample()      
