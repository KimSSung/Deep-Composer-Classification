...
with open('token_type.pkl', 'rb') as f:
...
num_labels = 5
config = BertConfig(vocab_size_or_config_json_file=131, hidden_size=36, num_hidden_layers=12, num_attention_heads=12, intermediate_size=1024)
...
with torch.no_grad():
	...
	for j, val_set in enumerate(val_loader):
		...
		before = model(input_ids=val_x_id, token_type_ids=val_x_tok, attention_mask=val_x_att)
		# print("valid output:",val_output)
		# print("pooled_output:", val_output)
		# print("after linear:", val_output)
		print("before softmax:", before)
		print("after softmax:", F.softmax(before, dim=1))
		print()
		# val_pred = torch.argmax(val_output, dim=1)
		# print("valid pred:", val_pred)
...
for epoch in range(num_epochs):
	...
	for i, trainset in enumerate(train_loader): #5(inputs each batch) x 640(batches)
		...
		output = model(input_ids=train_x_id, token_type_ids=train_x_tok, attention_mask=train_x_att)
		# print("train output:",output)

		#ACCURACY
		pred = torch.argmax(output, dim=1) #convert to prob that's sums upto 1 -> pick max value
		...
		if (i + 1) % 100 == 0:
			...
			with torch.no_grad():  # important!!! for validation
				...
				for j, val_set in enumerate(val_loader):
					...
					val_output = model(input_ids=val_x_id, token_type_ids=val_x_tok, attention_mask=val_x_att)
					# print("valid output:",output)

					#ACCURACY
					val_pred = torch.argmax(val_output, dim=1)  # convert to prob that's sums upto 1 -> pick max value
					print("val_pred:",val_pred, "    ||      val_y:", val_y)

					val_correct = val_pred.eq(val_y)  # if label is correct
					# print("val_correct:", val_correct)
					tot_val_correct += val_correct.sum().item()
					print("tot_val_correct | total_val_num:  ",tot_val_correct, " | ",len(val_input))
				...
				if tot_val_loss < best_val_loss:
					...
					filename = "../../../../data/models/model_valloss_" + str(val_loss)
			...
			print("epoch: {}/{} | step: {}/{} | train loss: {:.4f} | val loss: {:.4f} | train acc: {:.4f} | val acc: {:.4f}".format(
				...
				tot_val_correct / (len(val_input))
				))

			# print("tot val correct:", tot_val_correct, "   ||   val input:", len(val_input))
			...
			result['val_accuracy'].append(format(tot_val_correct/(len(val_input)),"10.2f"))

			tot_train_loss = 0.0
			tot_train_acc = 0.0

			model.train() # drop out on
