...
wave, sr = librosa.load(wav_file, sr=SAMPLING_RATE, mono=True, duration=30.0)
datasetXy(0, wave)
wave, sr = librosa.load(wav_file2, sr=SAMPLING_RATE, mono=True, duration=30.0)
datasetXy(1, wave)
print(X)
X = np.array(X)
print(X.shape)
...
'''
	...
	model = Sequential()
	model.add(Conv2D(64, (3, 3), padding='same', input_shape=(feature_dim_1, feature_dim_2, channel)))
