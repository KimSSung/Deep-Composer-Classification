...
#genre = ['Classical','Rock','Jazz','Counry','Pop']
genre = ['Classical']
#genre_dir = './midiset/' + genre

tensor_list = []
for genre_index in range (0,len(genre)):
    genre_dir = './../../../../data/midi820/' +  genre[genre_index] + '/'
    file_dir = []
    file_name = []
    ...
    for cur_file in range(0,len(file_dir)):
        ...
        print(num, ' - current: file :' + file_name[cur_file] + "< " + genre[genre_index] + ">")
        try:
        ...
        data.to_csv('./../../../../data/csv/'+file_name[cur_file] +'.csv' ,header=False, index = False)
