...
###model
pretrained_model = "/data/lstm_models/instr/1_model_valloss_13.229760706424713"
...
def test(model, test_loader, files, epsilon):
    ...
    for i, ((data, target), each_file) in enumerate(zip(test_loader, files)):
        ...
        if new_pred.item() == target.item():
            ...
            memory = 0 #default = piano
            #strochastic rounding to 0,1
            for i in range(adv_ex.shape[0]):
                for j in range(adv_ex.shape[1]):
                    if adv_ex[i][j] != 0: #if a cell is perturbed
                        rn = np.random.rand()
                        if(rn >= 0.9): # round!
                            orig_data[i][0][j] = memory #previous instr
                            memory = adv_ex[i][j]

            #check acc again
            orig_tensor = torch.from_numpy(orig_data).type(torch.Tensor)
            orig_tensor = orig_tensor.to(device)
            rounded_output = model(orig_tensor)
            rounded_pred = torch.max(rounded_output, 1)[1].view(target.size()).data
            # print("orig: {} after attack: {} after rounded attack: {}".format(target.item(),new_pred.item(),rounded_pred.item()))
            rounded_ex = orig_tensor.squeeze().detach().cpu().numpy()

            #save
            # adv_examples.append((init_pred.item(), new_pred.item(), adv_ex))
            # adv_orig.append(data)
            adv_fname.append(each_file)
            adv_rounded.append(rounded_ex)
    ...
    return final_acc, adv_rounded, adv_fname, orig_wrong, correct
...
for ep in epsilons:
    ...
    acc, rounded, fname, orig_wrong, correct = test(model, test_loader, FN, ep)
    print("Before attack: {} wrong, out of {} total data".format(orig_wrong, min_shape * len(genres)))
    print("After attack: {} still classified correctly, out of {} attempts".format(correct, min_shape * len(genres) - orig_wrong))
    ...
    #
    np.save("/data/midi820_fgsm/rounded_attacks_new/ep_"+str(ep)+"_fname", fname)   # save as .npy
    np.save("/data/midi820_fgsm/rounded_attacks_new/ep_"+str(ep)+"_rounded", rounded)   # save as .npy
