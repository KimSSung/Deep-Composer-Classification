...
fn, genres = read_data(gtzan_dir, genres) #get filename
...
class GenreClassModel(nn.Module):
	...
	#     )
	#
	#     self._classifier = nn.Sequential(nn.Linear(in_features=16384, out_features=1024),
	#                                      nn.ReLU(),
	#                                      nn.Dropout(),
	...
	#     x = self._extractor(x)
	#     x = x.view(x.size(0), -1)
	#     predictions = self._classifier(x)
	#     # print(predictions)
	#     return predictions
	#
	#
...
for epoch in tqdm(range(num_epochs)):
	...
	trn_correct = 0
	trn_total = 0
	for i, trainset in enumerate(train_loader): # len(train_loader) = 780
		#train mode
		model.train()
		#unpack
