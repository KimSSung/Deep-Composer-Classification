...
def main(config):
	...
	t = MIDIDataset(config.train_input_path, 0, config.genre_datanum * 0.8, config.genres, 'folder')
	v = MIDIDataset(config.valid_input_path, 0, config.genre_datanum * 0.2, config.genres, 'folder')
	...
	for epoch in range(config.epochs):
		...
		if epoch % val_term == 0:
			...
			if epoch == 0:
				avg_valloss, avg_valacc, val_file_names = Test(valid_loader, model, criterion, save_filename=True)
				# save val file names
				torch.save(val_file_names,config.valid_filename_save_path + 'val_file_names.pt')
				filenames = torch.load(config.valid_filename_save_path + 'val_file_names.pt')
				print('val file names len:',len(filenames))

			else:
				avg_valloss, avg_valacc, _ = Test(valid_loader, model, criterion, save_filename=False)
	   ...
	   val loss: {:.4f} | val acc: {:.2f}%'''
			...
			if avg_valloss < min_valloss:
				...
				torch.save({'epoch':epoch,
							...
							'loss':avg_valloss,
							'acc':avg_valacc}, config.model_save_path + config.model_name + '_valloss_' + str(float(avg_valloss)) + '_acc_' + str(float(avg_valacc)) + '.pt')
				print('model saved!')
