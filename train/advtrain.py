...
def main(config):
    ...
    t_list = []
    t_list.append(
        MIDIDataset(config.attacked_train_input_path, -1, -1, config.genres, "flat")
    )  # not use start, end index for 'flat'
    t_list.append(
        MIDIDataset(
            config.train_input_path,
            0,
            config.genre_datanum * 0.8,
            config.genres,
            "folder",
        )
    )
    ...
    v_list = []
    v_list.append(
        MIDIDataset(config.attacked_valid_input_path, -1, -1, config.genres, "flat")
    )
    v_list.append(v2)
    ...
    torch.save(
        adv_valid_loader_1, config.validloader_save_path + "adv_valid_loader_TandAT.pt"
    )
    print("adv_valid_loader_TandAT saved!")
    torch.save(
        adv_valid_loader_2, config.validloader_save_path + "adv_valid_loader_T.pt"
    )
    ...
    adv_valid_loader_1 = torch.load(
        config.validloader_save_path + "adv_valid_loader_TandAT.pt"
    )
    print("adv_valid_loader_TandAT loaded!")
    adv_valid_loader_2 = torch.load(
        config.validloader_save_path + "adv_valid_loader_T.pt"
    )
    ...
    for epoch in range(config.epochs):
        ...
        if epoch % val_term == 0:
            ...
            if epoch == val_term:
                avg_valloss_1, avg_valacc_1, val_file_names_1 = Test(
                    adv_valid_loader_1, model, criterion, save_filename=True
                )
                # save val file names
                torch.save(
                    val_file_names_1,
                    VALID_FILENAME_PATH + "adv_val_file_names_TandAT.pt",
                )
                ...
                avg_valloss_1, avg_valacc_1, _ = Test(
                    adv_valid_loader_1, model, criterion, save_filename=False
                )

            # 2. Only Test
            if epoch == val_term:
                avg_valloss_2, avg_valacc_2, val_file_names_2 = Test(
                    adv_valid_loader_2, model, criterion, save_filename=True
                )
                # save val file names
                torch.save(
                    val_file_names_2, VALID_FILENAME_PATH + "adv_val_file_names_T.pt"
                )
                filenames = torch.load(VALID_FILENAME_PATH + "adv_val_file_names_T.pt")
                print("Only Test val file names len:", len(filenames))

            else:
                avg_valloss_2, avg_valacc_2, _ = Test(
                    adv_valid_loader_2, model, criterion, save_filename=False
                )
