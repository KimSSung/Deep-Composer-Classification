...
model = LSTM( #hidden_dim = 80 best
    input_dim=129, hidden_dim=48, batch_size=batch_size, output_dim=len(genres), num_layers=2
)
loss_function = nn.NLLLoss()  # expects outputs from LogSoftmax
optimizer = optim.Adam(model.parameters(), lr=0.001) #0.0005 best for midi370
schedular = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min',factor=0.5,patience=10,verbose=True) #0.5 best for midi370
