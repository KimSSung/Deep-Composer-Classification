...
optimizer = optimization.BertAdam(model.parameters(), lr=0.001, t_total=3200)
		...
		if (i + 1) % 100 == 0:
			...
			with torch.no_grad():  # important!!! for validation
				...
				for j, val_set in enumerate(val_loader):
					...
					print("tot_val_correct:",tot_val_correct)
			...
			result['val_accuracy'].append(format(tot_val_acc/(len(val_input)),"10.2f"))
