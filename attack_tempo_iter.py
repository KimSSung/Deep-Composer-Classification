    ...
    for song_num in range(0, len(input)):
        ...
        while current_time < MAX_SIZE:
            ...
            if (current_time + multiplier * frac_time) > MAX_SIZE:
                break
            extend_time_fraction.append(
                [current_time, current_time + multiplier * frac_time]
            )
        ...
        for manipul in range(0, len(extend_time_fraction)):
            if manipul == 0:
                ...
                for fib in range(1, manipul + 1):
                    extend_time_fraction[manipul][0] += frac_time_list[fib] * (
                        multiplier_list[fib] - 1
                    )
                    extend_time_fraction[manipul][1] += frac_time_list[fib] * (
                        multiplier_list[fib] - 1
                    )
        ...
        for dec_idx in range(0, len(extend_time_fraction)):
            shorten_time_fraction.append(
                [
                    gathering_index_list[dec_idx],
                    gathering_index_list[dec_idx]
                    + frac_time_list[dec_idx] * multiplier_list[dec_idx],
                ]
            )
        ...
        for manipul in range(0, len(extend_time_fraction)):
            if manipul == 0:
                ...
                for fib in range(1, manipul + 1):
                    shorten_time_fraction[manipul][0] += (
                        shorten_time_fraction[fib - 1][1]
                        - shorten_time_fraction[fib - 1][0]
                    )
                    shorten_time_fraction[manipul][1] += (
                        shorten_time_fraction[fib - 1][1]
                        - shorten_time_fraction[fib - 1][0]
                    )
                    # if manipul == len(extend_time_fraction)-1:
        ...
        for channel in range(0, len(input[song_num])):
            ...
            for idx, time_index in enumerate(extend_time_fraction):
                ...
                append_numpy = temp[
                    time_index[0] : time_index[0] + frac_time_list[idx]
                ][:]
                ...
                for extend_index in range(0, frac_time_list[idx]):
                    for mul in range(0, multiplier_list[idx]):
                        new_numpy = np.vstack(
                            [new_numpy, append_numpy[extend_index][:]]
                        )
                        # print(len(new_numpy))
                    # print(len(new_numpy))
            ...
            for idx, time_index in enumerate(shorten_time_fraction):
                ...
                if idx == 0:
                    ...
                    checking.append(
                        [len(before_numpy), len(gathered_numpy), len(after_numpy)]
                    )
                else:
                    # print('Original time_index: ',shorten_time_fraction)

                    for decrease in range(1, idx + 1):
                        time_index[0] -= frac_time_list[decrease - 1] * (
                            multiplier_list[decrease - 1] - 1
                        )
                        time_index[1] -= frac_time_list[decrease - 1] * (
                            multiplier_list[decrease - 1] - 1
                        )
                ...
                for append_index in range(0, frac_time_list[idx]):
                    gathering_temp = gathered_numpy[
                        append_index * multiplier_list[idx]
                    ].copy()
                    for mul in range(1, multiplier_list[idx]):
                        ...
                        gathering_temp = np.maximum(
                            gathering_temp,
                            gathered_numpy[
                                append_index * multiplier_list[idx] + mul
                            ].copy(),
                        )
...
save_max_perturbed_tempo = np.ones(240 * 129 * 400 * 128).reshape(
    (240, 1, 129, 400, 128)
)
current_perturbed_tempo = np.ones(240 * 129 * 400 * 128).reshape(
    (240, 1, 129, 400, 128)
)
max_attack_acc = 100
