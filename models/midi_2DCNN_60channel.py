...
genres = ['Classical', 'Rock', 'Country', 'HipHopRap'] #best
num_genres = 4
min_shape= 820
batch_size = 20
...
for genre in genres:
    ...
    load_saved = np.load("/data/midi820_drum/" + genre + "_input.npy", allow_pickle=True)
...
# Define model
# model = resnet152(128, num_genres)
model = CustomCNN(128, num_genres)
# print(model)
        ...
        with torch.no_grad(): # important!!! for validation
            ...
            if avg_valloss < min_valloss:
                ...
                torch.save({'epoch':epoch,
                            ...
                            'loss':avg_valloss,
                            'acc':avg_valacc}, MODEL_SAVE_PATH + 'ResNet18_valloss_' + str(avg_valloss) + '_acc_' + str(avg_valacc) + '.pt')
                print('model saved!')
                # load:
...
model.eval()
checkpoint = torch.load('/data/midi820_128ch/model/bestmodel/ResNet18_valloss_0.5151_acc_85.0.pt')
...
print("val acc: {:.2f}%".format(val_acc/3))
'''