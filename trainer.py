...
class Trainer:
	...
	def train(self, mode):
		...
		for epoch in range(self.config.epochs + 1):
			...
			# TensorBoard
			# record running loss
			self.writer.add_scalar('training loss', trn_running_loss / self.num_batches, epoch)
	...
	def inputs_to_probs(net, inputs):
		'''
		Generate prediction & probs from (trained net, inputs)
		'''
		output = net(inputs)
	...
	def plot_classes_preds(net, inputs, labels):
		...
		- Else - red
		'''
		preds, probs = inputs_to_probs(net, inputs)
		# 배치에서 이미지를 가져와 예측 결과 / 정답과 함께 표시(plot)합니다
		fig = plt.figure(figsize=(12, 48))
		for idx in np.arange(4):
			ax = fig.add_subplot(1, 4, idx+1, xticks=[], yticks=[])
			matplotlib_imshow(inputs[idx], one_channel=True)
			ax.set_title("{0}, {1:.1f}%\n(label: {2})".format(
				classes[preds[idx]],
				probs[idx] * 100.0,
				classes[labels[idx]]),
