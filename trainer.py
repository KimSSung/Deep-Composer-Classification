...
# transform
import torchvision
from transpose import ToTensor
...
class Trainer:
	...
	def optim_selection(self):
		if self.config.optim == 'Nesterov':
			return optim.SGD(self.model.parameters(),lr=self.config.lr, momentum=0.9, nesterov=True)
		elif self.config.optim == 'SGD':
			return optim.SGD(self.model.parameters(),lr=self.config.lr, momentum=0.9, nesterov=False)
		elif self.config.optim == 'Adadelta': # default lr = 1.0
			return optim.Adadelta(self.model.parameters(), lr=self.config.lr, rho=0.9, eps=1e-06, weight_decay=1e-6)
		elif self.config.optim == 'Adagrad': # default lr = 0.01
			return optim.Adagrad(self.model.parameters(), lr=self.config.lr, lr_decay=0, weight_decay=1e-6, initial_accumulator_value=0, eps=1e-10)
		elif self.config.optim == 'Adam': # default lr=0.001
			return optim.Adam(self.model.parameters(), lr=self.config.lr, weight_decay=1e-6)
		elif self.config.optim == 'AdamW': # default lr=0.001
			return optim.AdamW(self.model.parameters(), lr=self.config.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.01, amsgrad=False)
		elif self.config.optim == 'SparseAdam': # default lr = 0.001
			return optim.SparseAdam(self.model.parameters(), lr=self.config.lr, betas=(0.9, 0.999), eps=1e-08)
		elif self.config.optim == 'Adamax': # default lr=0.002
			return optim.Adamax(self.model.parameters(), lr=self.config.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-6)
		elif self.config.optim == 'ASGD': 
			return optim.ASGD(self.model.parameters(), lr=self.config.lr, lambd=0.0001, alpha=0.75, t0=1000000.0, weight_decay=1e-6)
		elif self.config.optim == 'RMSprop': # default lr=0.01
			return optim.RMSprop(self.model.parameters(), lr=self.config.lr, alpha=0.99, eps=1e-08, weight_decay=0, momentum=0, centered=False)
		elif self.config.optim == 'Rprop': # default lr=0.01
			return optim.Rprop(self.model.parameters(), lr=self.config.lr, etas=(0.5, 1.2), step_sizes=(1e-06, 50))
	...
	def data_load(self, mode):
		if mode == "basetrain":
			...
			t = MIDIDataset(self.config.train_split_path, age=self.config.age, transform=torchvision.transforms.Compose([ToTensor()]))
			v = MIDIDataset(self.config.test_split_path, age=self.config.age, transform=torchvision.transforms.Compose([ToTensor()]))
	...
	val_T loss: {:.4f} | val_T acc: {:.2f}% """.format(
		...
		print("Accuracy: {:.4f} | Loss: {:.4f}""".format(self.best_valid[0], self.best_valid[1]))
