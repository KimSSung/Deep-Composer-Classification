...
class Trainer:
    def __init__(self, args):
        ...
        self.optimizer = optim.Adam(
            self.model.parameters(), lr=self.config.learning_rate, weight_decay=1e-6
        )  # 0.00005
        ...
        for epoch in range(self.config.epochs):
            ...
            if epoch % val_term == 0:
                ...
                if epoch == 0:
                    ...
                    if mode == "basetrain":
                        avg_valloss, avg_valacc, val_file_names = self.test(
                            self.valid_loader, self.model, save_filename=True
                        )
                        # save val file names
                        torch.save(
                            val_file_names,
                            self.config.valid_filename_save_path + "val_file_names.pt",
                        )
                        ...
                        avg_valloss_1, avg_valacc_1, val_file_names_1 = self.test(
                            self.valid_loader_1, self.model, save_filename=True
                        )
                        torch.save(
                            val_file_names_1,
                            self.config.valid_filename_save_path
                            + "adv_val_file_names_TandAT.pt",
                        )
                        filenames = torch.load(
                            self.config.valid_filename_save_path
                            + "adv_val_file_names_TandAT.pt"
                        ...
                        print(
                            "Test and Attack test val file names len:", len(filenames)
                        )

                        # 2. Only Test
                        avg_valloss_2, avg_valacc_2, val_file_names_2 = self.test(
                            self.valid_loader_2, self.model, save_filename=True
                        )
                        torch.save(
                            val_file_names_2,
                            self.config.valid_filename_save_path
                            + "adv_val_file_names_T.pt",
                        )
                        filenames = torch.load(
                            self.config.valid_filename_save_path
                            + "adv_val_file_names_T.pt"
                    ...
                    if mode == "basetrain":
                        avg_valloss, avg_valacc, _ = self.test(
                            self.valid_loader, self.model, save_filename=False
                        )

                    elif mode == "advtrain":
                        avg_valloss_1, avg_valacc_1, _ = self.test(
                            self.valid_loader_1, self.model, save_filename=False
                        )
                        avg_valloss_2, avg_valacc_2, _ = self.test(
                            self.valid_loader_2, self.model, save_filename=False
                        )
