...
class Trainer:
	...
	def model_selection(self):
		if self.config.model_name == "resnet18":
			return resnet18(in_channels=int(self.config.input_shape[0]), num_classes=self.label_num)
		elif self.config.model_name == "resnet34":
			return resnet34(in_channels=int(self.config.input_shape[0]), num_classes=self.label_num)
		elif self.config.model_name == "resnet50":
			return resnet50(in_channels=int(self.config.input_shape[0]), num_classes=self.label_num)
		elif self.config.model_name == "resnet101":
			return resnet101(in_channels=int(self.config.input_shape[0]), num_classes=self.label_num)
		elif self.config.model_name == "resnet152":
			return resnet152(in_channels=int(self.config.input_shape[0]), num_classes=self.label_num)
		elif self.config.model_name == "convnet":
			return convnet(in_channels=int(self.config.input_shape[0]), num_classes=self.label_num)
	...
	def optim_selection(self):
		if self.config.optim == "Nesterov":
			return optim.SGD(
				self.model.parameters(), lr=self.config.lr, momentum=0.9, nesterov=True, weight_decay=0.0005
			)
		elif self.config.optim == "SGD": # weight_decay = l2 regularization
			return optim.SGD(
				self.model.parameters(), lr=self.config.lr, momentum=0.9, nesterov=False, weight_decay=0.005
			)
	...
	def data_load(self, mode):
		...
		if mode == "basetrain":
			...
			# )
			self.train_loader = DataLoader(
				t, batch_size=self.seg_num, shuffle=True
			)
			self.valid_loader = DataLoader(
				v, batch_size=self.seg_num, shuffle=False
			)
